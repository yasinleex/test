...
-o 后跟生成的文件名
-v 在将配置文件转换为firmware文件时可用此参数插入一个十六进制的版本号字段fw_ver(0x00~0xFF),这是可选参数，如果没有此参数则生成的firmware中的fw_ver字段为0x00
-c 在将配置文件转换为firmware文件时可用此参数插入一个十六进制的芯片标识字段chip_type,用来区分hx9023s和hx9031as,规定该值为0x00时表示hx9023s，为0x01时表示hx9031as，这是可选参数，如果没有此参数则生成的firmware中的chip_type字段为0x00，表示hx9023s；该字段用于将驱动切换为适配hx9031as，为1时表示驱动走hx9031as的逻辑，为0x00时表示驱动走hx9023s的逻辑
示例：
...
8.-c和-v参数后跟的值必须是0x开头的十六进制数，不支持十进制输入！
